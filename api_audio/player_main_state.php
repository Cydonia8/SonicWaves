<?php
    ...
    $id_grupo_recomendado = $conexion->query("SELECT id from grupo where id <> 0 and foto is not null order by rand() limit 1");
    ...
    // }else{
        // $sentencia_grupo_recomendado->close();
        $sentencia_grupo_recomendado_v2 = $conexion->prepare("SELECT nombre, foto, id from grupo where id = ? and foto is not null");
        $sentencia_grupo_recomendado_v2->bind_param('i', $id);
        $sentencia_grupo_recomendado_v2->bind_result($nombre, $recomendado, $grupo);
    ...
    $datos_recogidos = [];
    $sentencia_albumes = $conexion->query("select a.id id, titulo, a.foto foto, nombre autor from album a, grupo g where a.grupo = g.id");
